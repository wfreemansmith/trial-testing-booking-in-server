...
- RBAC + tokenised links — least privilege, time-limited access
...
| **Security & Governance** | RBAC + tokens. Compensating transaction in `submit()`. HTTP error hierarchy | K13, S4 |
...
*Last updated: after reading AM1 criteria guidance xlsx (18 ACs with distinction criteria) and report template docx. Session topics: `submit()` implementation, Files.com upload, compensating transaction pattern, error handling, report review.*
