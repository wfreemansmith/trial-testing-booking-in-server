from fastapi import APIRouter, UploadFile, Form, File, Body, HTTPException, status, Depends
from src.controllers import upload_controller
from src.utils import api_response
from src.auth import require_centre_permission, require_permission
from src.schemas.upload_schema import parse_upload_data, parse_uploadfile_data, UploadPayload
import json, os, uuid, shutil, tempfile
from src.config import STAGING_DIR
from src.errors import UnprocessableEntity

router = APIRouter()

@router.get("/")
@api_response()
def get_context(user = Depends(require_permission("upload:read"))):
    """Once users first login to the page retrieves basic information about their centre"""
    return {
        "display_name": user.display_name,
        "centre_id": user.centre_contact.centre_id if user.centre_contact else None,
        "centre_name": user.centre_contact.centre.centre_name if user.centre_contact else None,
        "marking_window_id": user.marking_window_id,
        "window_name": user.marking_window.window_name if user.marking_window else None,
        "role": user.role.role_name
    }


@router.post("/preview")
@api_response()
async def preview_upload(
    file: UploadFile = File(...),
    user = Depends(require_centre_permission("upload:write"))
    ):
    """Process register and return data contained within"""
    if file.filename.endswith('.xlsx'):
        file_bytes = await file.read()
    else:
        raise HTTPException(
            status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
            detail=f"File type .{file.filename.split(".")[-1]} not supported"
        )

    return upload_controller.preview(
        centre_id=user.centre_contact.centre_id,
        marking_window_id=user.marking_window_id,
        file=file_bytes
        )


@router.post("/refresh")
@api_response()
def refresh_data(
    payload: UploadPayload,
    user = Depends(require_centre_permission("upload:read"))
    ):
    """Validate user inputted data and returns any error flags"""
    data = payload.data
    parsed_data = parse_upload_data(data)
    return upload_controller.check(
        centre_id=user.centre_contact.centre_id,
        marking_window_id=user.marking_window_id,
        data=parsed_data)


@router.post("/file_upload")
@api_response()
async def file_upload(
    data: str = Form(...),
    file: UploadFile = File(...),
    user = Depends(require_centre_permission("upload:write"))
    ):
    """Upload scanned materials to staging directory for later upload to files.com"""
    parsed_data = parse_uploadfile_data(json.loads(data))

    if not file.filename.endswith('.pdf'):
        raise HTTPException(
            status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE,
            detail=f"File type .{file.filename.split(".")[-1]} not supported"
        )
    
    temp_filename = f"{uuid.uuid4()}.pdf"
    temp_path = os.path.join(STAGING_DIR, temp_filename)

    file.file.seek(0)
    with open(temp_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    
    return upload_controller.stage_file(
        centre_id=user.centre_contact.centre_id,
        marking_window_id=user.marking_window_id,
        batch=parsed_data.batch,
        candidates=parsed_data.candidates,
        temp_path=temp_path
    )


@router.post("/submit")
@api_response()
def submit_upload(
    payload: UploadPayload,
    user = Depends(require_centre_permission("upload:write"))
    ):
    """Checks all data and uploads, and if no errors commits to database"""
    data = payload.data
    parsed_data = parse_upload_data(data)
    checked_data = upload_controller.check(
        centre_id=user.centre_contact.centre_id,
        marking_window_id=user.marking_window_id,
        data=parsed_data,
        check_file_upload=True)

    if checked_data['errors']:
        raise UnprocessableEntity(message="Could not process upload", data=checked_data)
    else:
        upload_controller.submit(
            centre_id=user.centre_contact.centre_id,
            marking_window_id=user.marking_window_id,
            data=parsed_data
            )